  ...
  <div class="card-mobile" @click="handleNavigation">
    ...
    <div class="content-info">
      ...
      <div class="ratings-wrapper">
        ...
        >
          <div class="rated" v-if="data?.userRating?.value">
            <i class="fa-solid fa-star solid-star-icon"></i>
            <b id="rating">{{ data?.userRating?.value }}<span>/10</span></b>
          </div>
          <i
            :class="[
              'fa-regular',
    ...
    const handleOpenRatingModal = () => {
      if (user.value?.id) {
        ...
        store.commit("SET_USER_RATING", {
          _id: props.data?.userRating?._id,
          rateNum: props.data?.userRating?.value,
        });
        store.commit("SET_RATING_MODAL_DATA", {
          contentId: props.data?._id,
          title: props.data?.title,
          route: { name: props.data?.type, param: "" },
        });
      }
    };
